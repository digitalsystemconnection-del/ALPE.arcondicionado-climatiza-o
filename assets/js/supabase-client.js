...
async function adicionarAoCarrinho(produtoId, quantidade = 1) {
  ...
  const { data, error } = await supabase
    .from('carrinhos')
    .insert({ session_id: sessionId, produto_id: produtoId, quantidade: quantidade })
    .select();
...
async function criarPedido(dadosCliente) {
  ...
  if (pedidoError) { console.error('Erro ao criar pedido:', pedidoError); return null; }
...
// ============================================
// EXPORTAR - GARANTIR QUE ESTÁ DISPONÍVEL
// ============================================
window.supabaseAPI = {
...
console.log('✅ SupabaseAPI carregado com sucesso!');
