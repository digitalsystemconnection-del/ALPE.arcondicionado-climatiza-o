...
async function carregarProdutosEmDestaque(limite = 12) {
  const { data, error } = await supabase.from('produtos').select('*').eq('ativo', true).eq('destaque', true).limit(limite);
...
async function adicionarAoCarrinho(produtoId, quantidade = 1) {
  const sessionId = localStorage.getItem('session_id') || 'session_' + Date.now();
  const { data, error } = await supabase.from('carrinhos').insert({ session_id: sessionId, produto_id: produtoId, quantidade: quantidade }).select();
...
async function criarPedido(dadosCliente) {
  ...
  const numeroPedido = 'ALPE-' + Date.now().toString().slice(-8) + '-' + Math.random().toString(36).substr(2, 4).toUpperCase();
  const sessionId = localStorage.getItem('session_id') || 'session_' + Date.now();
  ...
  if (pedidoError) { console.error('Erro:', pedidoError); return null; }
  
  const itensParaInserir = itens.map(item => ({ pedido_id: pedido.id, ...item }));
